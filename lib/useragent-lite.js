...
// app patterns
const applications = [
  { name: 'Google Search App', flags: ['inApp'], regex: /(?:GSA)\/([0-9]+)\./i },
  { name: 'Facebook', flags: ['inApp'], regex: /\[FB.*\/(\d+)(?:\.(\d+)(?:\.(\d+))?)?/ },
  { name: 'Instagram', flags: ['inApp'], regex: /(?:Instagram.*)([0-9]+)/i },
  { name: "Unknown", flags: ["isUnknown"], regex: null }
];
...
function matchApp(ua, s) {
  applications.some(app => {
    if (!app.regex || app.regex.test(s)) {
      ua.application = app.name;
